...
const Preview = ({ name, number, email }) => {
          ...
          <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-md overflow-hidden">
            <div className="p-4 sm:p-6">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-semibold">{name.trim().length > 0 ? name : "Mike Tyson"}</h2>
                  <p className="text-sm text-gray-600">Senior Software Engineer</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Phone:{number.trim().length > 0 ? number : "(123) 456-7890"}</p>
                  <p className="text-sm text-gray-600">Email:<a href={email.trim().length > 0 ? email : "#"}> {email.trim().length > 0 ? email : "mike@example.com"}</a></p>
